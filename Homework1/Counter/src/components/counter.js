...
class Counter extends React.Component {
  ...
  componentDidUpdate() {
    ...
    if (!storageCount) {
      window.localStorage.setItem(
        "count",
        JSON.stringify({ count: this.state.count })
      );
    } else {
  ...
  handleAdd = (step) => {
    this.setState({
      count:
        this.state.count + step > this.state.max
          ? +this.state.max
  ...
  handleSub = (step) => {
    this.setState({
      count:
        this.state.count - step < this.state.min
          ? +this.state.min
          : this.state.count - step,
    });
  };
  ...
  render() {
    return (
      <div className="container">
        ...
        <button
          onClick={() => {
            this.handleSub(+this.state.step);
          }}
          disabled={this.state.count == this.state.min}
        >
          -
        </button>
        <div>
          ...
          this.state.count >= this.state.min
            ? this.state.count
            : this.state.min}
        </div>
        <button
          onClick={() => {
            this.handleAdd(+this.state.step);
          }}
          disabled={this.state.count == this.state.max}
        >+</button>
